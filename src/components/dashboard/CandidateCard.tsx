...
const CandidateCard: React.FC<CandidateCardProps> = ({ 
  ...
  const handleCardClick = (e: React.MouseEvent<HTMLDivElement>) => {
    // Don't open modal if clicking on the "Show More/Less" button or its children
    const target = e.target as HTMLElement;
    const isShowMoreClick = 
      target.closest('button') && 
      (target.closest('button')?.textContent?.includes('Show More') || 
       target.closest('button')?.textContent?.includes('Show Less'));
